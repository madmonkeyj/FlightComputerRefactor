...
bool NavigationManager_UpdateGPS(const GPS_Data_t* gps_data) {
    ...
    /* Initialize EKF on first GPS fix */
    if (nav_ctx.state == NAV_STATE_AHRS_ONLY) {
        /* Use GPS altitude if valid, otherwise use estimate */
        float reference_altitude = (gps_data->altitude > 5.0f && gps_data->altitude < 1000.0f) ?
                                  gps_data->altitude : 128.6f;

        /* Store reference - CRITICAL: Use the exact same GPS data for both reference and first measurement */
        nav_ctx.solution.gps_ref_lat = gps_data->latitude;    // Store exact values
        nav_ctx.solution.gps_ref_lon = gps_data->longitude;   // Store exact values
        ...
        if (SensorAdapter_Read(&current_sensors) && current_sensors.baro_valid) {
            ...
            /* Initialize EKF with the SAME GPS data used for reference */
            if (NavEKF_Init(&nav_ctx.ekf, nav_ctx.config.rates.ekf_hz) == HAL_OK) {
                if (NavEKF_SetGPSReference(&nav_ctx.ekf, gps_data->latitude,
                    ...
                    // 🔧 IMMEDIATELY apply the same GPS data as the first measurement
                    // This ensures perfect consistency between reference and first measurement
                    NavEKF_UpdateGPS(&nav_ctx.ekf, gps_data->latitude, gps_data->longitude, gps_data->altitude);

                    // Apply GPS velocity if available
    ...
    /* Continue with normal GPS updates for operational state */
    if (nav_ctx.state == NAV_STATE_OPERATIONAL) {
        /* Position update */
        NavEKF_UpdateGPS(&nav_ctx.ekf, gps_data->latitude, gps_data->longitude, gps_data->altitude);
