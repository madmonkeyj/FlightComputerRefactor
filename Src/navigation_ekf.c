...
static HAL_StatusTypeDef EKF_MeasurementUpdate(NavigationEKF_t *nav_ekf,
    ...
    // **CAPTURE INNOVATION VALUES FOR DEBUGGING**
    ...
    nav_ekf->debug.last_debug_update = HAL_GetTick();
...
MotionState_t NavEKF_DetectMotionState(NavigationEKF_t *nav_ekf,
    ...
    MotionSensorReading_t sensor_reading = {
        .accel_magnitude = sqrtf(accel_body_x * accel_body_x +
                                accel_body_y * accel_body_y +
                                accel_body_z * accel_body_z),
        .accel_variance = fabsf(sensor_reading.accel_magnitude - 9.80665f),
        .gyro_activity = sqrtf(gyro_x * gyro_x + gyro_y * gyro_y + gyro_z * gyro_z),
        .sensors_valid = true
    };
...
HAL_StatusTypeDef NavEKF_PredictWithMotionDetection(NavigationEKF_t *nav_ekf,
    ...
    /* REFACTORED: Use coordinate transformation module */
    float accel_body[3] = {accel_body_x, accel_body_y, accel_body_z};
    CoordTransformResult_t coord_result;

    HAL_StatusTypeDef coord_status = CoordTransform_BodyToNED(attitude,
                                                             accel_body,
                                                             &nav_ekf->accel_cal,
                                                             &coord_result);
    ...
    /* UNCHANGED: State prediction (keep all existing code from here down) */
