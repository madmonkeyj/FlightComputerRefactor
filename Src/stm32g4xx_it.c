...
void HardFault_Handler(void)
{
  ...
  while (1)
  {
    /* USER CODE BEGIN W1_HardFault_IRQn 0 */
      ...
      // crude delay loop, avoids relying on SysTick/HAL_Delay
...
void EXTI9_5_IRQHandler(void)
{
  ...
  HAL_GPIO_EXTI_IRQHandler(INT_BMP_Pin);
...
void USART1_IRQHandler(void)
{
  ...
  if(__HAL_UART_GET_FLAG(&huart1, UART_FLAG_IDLE) != RESET)
  {
    ...
    extern void BLE_UART_RxEventCallback(void);
...
/**
  * @brief SPI Transmit Receive Complete callback (for IMU DMA).
  */
void HAL_SPI_TxRxCpltCallback(SPI_HandleTypeDef *hspi) {
    if (hspi == &hspi3) {
        SensorManager_IMU_DMA_Complete_Callback();
    }
}

/**
  * @brief SPI Receive Complete callback (for IMU DMA if using Receive only).
  */
...
/**
  * @brief I2C Memory Rx Transfer completed callback (for I2C DMA with arbiter).
  * UPDATED: Now calls arbiter handler to route to correct device
  */
void HAL_I2C_MemRxCpltCallback(I2C_HandleTypeDef *hi2c) {
    if (hi2c == &hi2c1) {
        /* Arbiter handles routing to MAG, BARO, or High-G based on current device */
        I2C_DMA_Arbiter_HandleComplete(hi2c);
    }
}

/**
  * @brief I2C error callback.
  * UPDATED: Now calls arbiter error handler
  */
void HAL_I2C_ErrorCallback(I2C_HandleTypeDef *hi2c)
{
    if (hi2c == &hi2c1) {
        /* Arbiter handles error cleanup */
        I2C_DMA_Arbiter_HandleError(hi2c);
    }
}
...
{
    if (hspi == &hspi3) {
        // Clear the busy flag to prevent infinite hang
        SensorManager_IMU_DMA_Complete_Callback();
    }
}

/**
  * @brief GPIO EXTI callback (for data ready interrupts).
  */
